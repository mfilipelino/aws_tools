    ...
    for page in page_iterator:
        for table in page.get("TableList", []):
            ...
            if storage_desc:
                ...
                result["input_format"] = (
                    storage_desc.get("InputFormat", "").split(".")[-1] if storage_desc.get("InputFormat") else ""
                )
                result["output_format"] = (
                    storage_desc.get("OutputFormat", "").split(".")[-1] if storage_desc.get("OutputFormat") else ""
                )
...
def cli(database, prefix, profile, region, output_format, limit, output_fields, no_header, verbose):
    ...
    """
    # Get tables
    tables = list_athena_tables(database=database, prefix=prefix, profile=profile, region=region, verbose=verbose)
