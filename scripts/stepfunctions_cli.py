...
def main():
    parser = argparse.ArgumentParser(
        description="Check Step Function executions and optionally generate rerun commands for failed ones."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Flag to generate rerun commands for failed executions.",
    )
    ...
    print(
        f"Checking for failed executions for state machine {args.state_machine_arn} since {start_date_threshold.isoformat()}..."
    )
    ...
    try:
        ...
        for execution in executions_data["executions"]:
            ...
            try:
                ...
                print(
                    f"Error parsing date for execution {execution['executionArn']}: {execution['startDate']}. Error: {e}"
                )
