...
# Patch boto3.Session before importing the modules so that module-level code
# does not try to load AWS configuration.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

with patch("boto3.Session") as mock_session:
    mock_session.return_value.client.return_value = MagicMock()
    # Import CLI commands after mocking
    ...
    from cli.list_stepfunctions import cli as sfn_cli
...
class TestCLICommands(unittest.TestCase):
    ...
    def test_s3_list_basic(self, mock_client):
        ...
        # Test the command
        result = self.runner.invoke(s3_cli, ["--bucket", "test-bucket", "--prefix", "test-prefix-", "--format", "json"])
    ...
    def test_glue_list_basic(self, mock_client):
        ...
        result = self.runner.invoke(glue_cli, ["--prefix", "etl-", "--format", "json"])
    ...
    def test_kinesis_list_basic(self, mock_client):
        ...
        # Mock response
        mock_paginator.paginate.return_value = [{"StreamNames": ["clickstream-prod", "clickstream-dev"]}]

        result = self.runner.invoke(kinesis_cli, ["--prefix", "clickstream-", "--format", "json"])
        ...
        mock_sfn_client.get_paginator.return_value.paginate.return_value = [
            {
                "stateMachines": [
                    {
                        "stateMachineArn": "arn:aws:states:us-east-1:123:stateMachine:MachineA",
                        "name": "MachineA",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1, 12, 0, 0),
                    },
                    {
                        "stateMachineArn": "arn:aws:states:us-east-1:123:stateMachine:MachineB",
                        "name": "MachineB",
                        "type": "EXPRESS",
                        "creationDate": datetime(2024, 1, 2, 12, 0, 0),
                    },
                ]
            }
        ]
        ...
        mock_sfn_client.get_paginator.return_value.paginate.return_value = [
            {
                "stateMachines": [
                    {
                        "stateMachineArn": "arn:sf:sm:TestMachineA",
                        "name": "TestMachineA",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1),
                    },
                    {
                        "stateMachineArn": "arn:sf:sm:AnotherMachineB",
                        "name": "AnotherMachineB",
                        "type": "EXPRESS",
                        "creationDate": datetime(2024, 1, 1),
                    },
                ]
        ...
        mock_sfn_client.get_paginator.return_value.paginate.return_value = [
            {
                "stateMachines": [
                    {
                        "stateMachineArn": "arn:sf:sm:Machine-01-Prod",
                        "name": "Machine-01-Prod",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1),
                    },
                    {
                        "stateMachineArn": "arn:sf:sm:Machine-02-Dev",
                        "name": "Machine-02-Dev",
                        "type": "EXPRESS",
                        "creationDate": datetime(2024, 1, 1),
                    },
        ...
        mock_sfn_client.get_paginator.return_value.paginate.return_value = [
            {
                "stateMachines": [
                    {
                        "stateMachineArn": "arn:sf:sm:TaggedMachine",
                        "name": "TaggedMachine",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1),
                    },
                    {
                        "stateMachineArn": "arn:sf:sm:UntaggedMachine",
                        "name": "UntaggedMachine",
                        "type": "EXPRESS",
                        "creationDate": datetime(2024, 1, 1),
                    },
                ]
        ...
        mock_sfn_client.get_paginator.return_value.paginate.return_value = [
            {
                "stateMachines": [
                    {
                        "stateMachineArn": "arn:sf:sm:Machine1",
                        "name": "Machine1",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1),
                    },  # Matches all tags
                    {
                        "stateMachineArn": "arn:sf:sm:Machine2",
                        "name": "Machine2",
                        "type": "EXPRESS",
                        "creationDate": datetime(2024, 1, 1),
                    },  # Matches one tag
                    {
                        "stateMachineArn": "arn:sf:sm:Machine3",
                        "name": "Machine3",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1),
                    },  # Matches no tags
        ...
        mock_sfn_client.get_paginator.return_value.paginate.return_value = [
            {
                "stateMachines": [
                    {
                        "stateMachineArn": "arn:sf:sm:MachineX",
                        "name": "MachineX",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1),
                    },
        ...
        mock_sfn_client.get_paginator.return_value.paginate.return_value = [
            {
                "stateMachines": [
                    {
                        "stateMachineArn": "arn:sf:sm:MachineY",
                        "name": "MachineY",
                        "type": "STANDARD",
                        "creationDate": datetime(2024, 1, 1),
                    },
